...
`docs/`는 GitHub Pages 게시 루트입니다. 원문 파일, ZIP 해제본, OCR 결과는 포함하지 않고 재현 가능한 경량 집계와 인벤토리를 게시합니다. `type-examples/`에는 문서 내용 분석 대상 여부를 검토할 수 있는 유형별 대표 문서를 최대 3건, 각 문서의 저해상도 발췌 페이지를 최대 5장 포함하며, 혼재한 PDF·HWP/HWPX 형식도 함께 보여 줍니다.
