...
const legacy = JSON.parse(fs.readFileSync(path.join(root, 'docs/data/type_samples.json')));
...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#cards .sample').count(), legacy.samples.filter(s => s.source_id === 'publications').length);
    await page.locator('[data-pub-status="excluded"]').click();
    assert.equal(await page.locator('#cards .sample').count(), 1);
